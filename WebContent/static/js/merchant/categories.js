	...
	$(".bg-model-update-ok").click(
			function() {
				var url = sys.contextPath
						+ "/merchants/merchant/categories/updateCategories";
				var params = {};
				...
				request.ajax(url, params, true, function(data) {
					if (data.success) {
						window.location.href = sys.contextPath
								+ "/merchants/merchant/categories/list";
					} else {
						// alert("更新失败")
					}
				});
					...
					function() {
						...
						if (goal == "catalog") {
							url1 = sys.contextPath
									+ "/merchants/merchant/categories/addCatalog";
							$temp = $if;
						} else if (goal == "secondcatalog") {
							url1 = sys.contextPath
									+ "/merchants/merchant/categories/addSecondCatalog";
							params[$iadd.attr("name")] = $iadd.val();
							$temp = $is;
						} else {
							url1 = sys.contextPath
									+ "/merchants/merchant/categories/addSecondValue";
							params[$iadd.attr("name")] = $iadd.val();
							$temp = $iv;
						}

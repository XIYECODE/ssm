...
$(function(){
	...
	// 发送请求
	var queryUrl = sys.contextPath+'/merchants/merchant/orderItems/list';
	request.ajax(queryUrl, {"status":$("#status").val()}, true, function(data){
		var rows = data["rows"];
	   	...
	   	$("#user_id").children().not(":first").remove();
	   	for(var i=0; i<rows.length; i++){
		   	...
		   	if(!flag_userId.hasOwnProperty(rows[i]["username"])){					// 剔除重复性订单号
			   	flag_userId[rows[i]["username"]] = true;
	 		   	$("#user_id").append('<option value="'+rows[i]["sid"]+'">'+rows[i]["username"]+'</option>');
		   	}
	   	}
	   	if(rows[0]['status'] == '2'){
	   		$("#toolbar").append('<button type="button" class="btn btn-success btn-flat  fahuo">发货</button>');
	   	}
	 	// 渲染数据
   ...
   function actionFormatter(value, row, index) {
       ...
       if(row.status == '2'){
       	// 三个id的循序不能倒，分割符也只能为-
           result += '<button type="button" class="btn btn-success" id="'+row.id+'-'+row.pid+'-'+row.sid+'" onclick="option2(this);">发货</button>';
       }
       return result;
   }
   ...
   function userFormatter(value, row, index){
   	return '<a href="#" id="'+row.sid+'">'+row.username+'</a>';
   }
});
	

$("body").on("change", "#order_id, #user_id", "#user_id", function(){
	...
	var url = sys.contextPath + "/merchants/merchant/orderItems/list";
	$("#product_table").bootstrapTable('refresh', {query:param});
});
...
$("body").on("click", "#toolbar > button", function(){
	...
	var url = sys.contextPath + "/merchants/merchant/orderItems/orderItems.do";
	var params = {"status":3};
	params["ids"] = ids;

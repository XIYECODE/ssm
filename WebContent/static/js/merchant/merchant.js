...
$(function(){
	if($("#status").val() == "1"){
		 $("#forbid").removeClass("hidden");
		 $("#unforbid").addClass("hidden");
	 }else  if($("#status").val() == "9"){
		 $("#unforbid").removeClass("hidden");
		 $("#forbid").addClass("hidden");
	 }
	...
	$("#toolbar > button").on("click",  function(){
		...
		if("edit" == ope){
			window.location.href = sys.contextPath+"/merchants/"+idArray[0]+"/editUI";// ${pageContext.request.contextPath}放在js中不好，当js代码从页面抽离出来时，就会失效
		}else if("delete" == ope){
			request.ajax(sys.contextPath+"/merchants/"+ids, "_method=DELETE", true, function(){
				$table.bootstrapTable('refresh');		//渲染页面
			});
		}else if("forbid" == ope){
			request.ajax(sys.contextPath+"/merchants/"+ids, {"_method":"PUT", "status":9}, true, function(){
				$table.bootstrapTable('refresh');		//渲染页面
			});
		}else if("unforbid" == ope){
			request.ajax(sys.contextPath+"/merchants/"+ids, {"_method":"PUT", "status":1}, true, function(){
				$table.bootstrapTable('refresh');		//渲染页面
			});
		}
	} );
